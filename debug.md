Trace: pobieranie danych do wykresu porównania i rysowanie (statyczny debug)

Kontekst: ComparisonView w app/ui/comparison_view_server.py, użytkownik wybiera kilka gier i klika "Porównaj wybrane" (zakres czasu np. 7d). Poniżej opis przebiegu od kliknięcia do narysowania wykresu z przybliżeniem, jakie obiekty/pamięć pojawiają się po drodze. To nie jest pomiar runtime, a uporządkowany ślad z kodu – miejsca do podglądu w debuggerze są wskazane.
Kroki z danymi i pamięcią

    Klik przycisku Porównaj wybrane (_compare_btn.clicked → asyncio.create_task(self._load_comparison()))
    ...
    Wejście do ComparisonView._load_comparison
    ...
    Wywołanie ServerClient.get_player_history_comparison(appids, days)
    ...
    AuthenticatedAPIClient.ensure_authenticated (jeśli brak tokenu lub wygasł)
    ...
    ServerClient.get_player_history_comparison
    ...
    Powrót w ComparisonView._load_comparison
    ...
    _update_chart
        ...
        self._figure.tight_layout() + self._canvas.draw() – rasteryzacja/rysowanie: tworzy bufory obrazu (typowo RGBA, szerokośćwysokość4 bajty; dla figury ~1000x600 to ~2.4 MB na bufor) + struktury Qt dla widgetu.
