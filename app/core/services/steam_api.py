...
class SteamWebApiClient(BaseAsyncService, ISteamWebApi):
    ...
    def _parse_hours_to_minutes(self, s: Optional[str]) -> int:
        if not s:
            return 0
        # Keep digits and separators, drop spaces/letters
        raw = re.sub(r"[^0-9,\.]", "", s)
        if not raw:
            return 0
        # If both separators present, treat the last one as decimal, others as thousands
        if "," in raw and "." in raw:
            last_sep_pos = max(raw.rfind(","), raw.rfind("."))
            dec_sep = raw[last_sep_pos]
            # remove all separators, then reinsert decimal point
            digits_only = re.sub(r"[,.]", "", raw)
            if last_sep_pos >= 0 and last_sep_pos < len(raw) - 1:
                # number of digits after decimal
                frac_len = len(raw) - last_sep_pos - 1
                if frac_len > 0:
                    int_part = digits_only[:-frac_len] if frac_len < len(digits_only) else "0"
                    frac_part = digits_only[-frac_len:]
            ...
            # Only one (or none) type of separator present
            if "," in raw:
                # If single comma and less than or equal to 2 digits after -> decimal, else thousands
                parts = raw.split(",")
    ...
    async def resolve_steamid(self, input_str: str) -> Optional[str]:
        """Resolve SteamID64 from numeric id, vanity id or profile URL (public, no API key required)."""
        s = (input_str or "").strip()
        if not s:
            return None
        # numeric SteamID64
        if s.isdigit() and 16 <= len(s) <= 20:
            return s
        # URL cases
            ...
            try:
                ...
                if len(parts) >= 2 and parts[0] in ("profiles", "id"):
                    ...
                    if kind == "id" and value:
                        # vanity -> query XML to resolve steamID64
                        xml_url = f"https://steamcommunity.com/id/{value}?xml=1&l=polish"
                        async with httpx.AsyncClient(timeout=10.0) as client:
                            r = await client.get(xml_url)
                            if r.status_code == 200:
                                try:
                                    root = ET.fromstring(r.text)
                                    sid = root.findtext("steamID64")
