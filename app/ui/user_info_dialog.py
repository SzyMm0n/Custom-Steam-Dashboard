...
class SteamUserInfoDialog(QDialog):
    """Okno informacji o uzytkowniku Steam (ASCII labels only).

    Akceptuje: SteamID64, vanity name lub pelny URL profilu.
    API Key opcjonalny.
    """

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Logowanie / Informacje o uzytkowniku Steam")
    ...
    def _init_ui(self) -> None:
        ...
        # Ustaw domy?ln? czcionk? z obs?ug? PL na ca?y dialog
        ...
        self.api_key_input.setPlaceholderText("Jezeli puste, uzyte zostana publiczne dane profilu")
        ...
        # Styl – wymu? czcionk? z pe?nymi znakami PL (fallback w CSS)
        self.setStyleSheet(
            """
    ...
    async def _on_fetch_clicked(self) -> None:
        ...
        self.status_lbl.setText("Rozwiazywanie profilu...")
        self.persona_lbl.setText("Ladowanie...")
        self.avatar_lbl.clear()
        self.table.setRowCount(0)

        # Resolve to SteamID64 first
        try:
            async with SteamWebApiClient(api_key=api_key) as api:
                steamid = await api.resolve_steamid(raw_input)
        except Exception as e:
            self.status_lbl.setText(f"Blad podczas rozwiazywania: {e}")
            self.fetch_btn.setEnabled(True)
            return
        if not steamid:
            self.status_lbl.setText("Nie udalo sie wykryc SteamID64 z podanego wejscia.")
            self.fetch_btn.setEnabled(True)
            return
        ...
        try:
            ...
            self.status_lbl.setText(f"Blad: {e}")
            self.fetch_btn.setEnabled(True)
            return
        ...
        for row, g in enumerate(owned_sorted):
            ...
            # If parsing produced inconsistent values (last2w > total), fix up total >= last2w
        ...
        self.status_lbl.setText(f"Zaladowano gier: {len(owned_sorted)}")
        self.fetch_btn.setEnabled(True)
