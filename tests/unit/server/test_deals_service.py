...
class TestGetGamePrices:
    ...
    async def test_get_game_prices_success(self):
        ...
        # Mock the lookup response (_get_json)
        mock_lookup_data = {
            "game": {
                "id": "gtav",
        ...
        # Mock the prices response (_client.post)
        ...
        mock_prices_response.raise_for_status = Mock()

        with patch.object(client, '_get_json', new_callable=AsyncMock) as mock_get_json:
            mock_get_json.return_value = mock_lookup_data

            with patch.object(client._client, 'post', new_callable=AsyncMock) as mock_post:
                mock_post.return_value = mock_prices_response

                # Use paid game AppID (271590 = GTA V) instead of F2P game (730 = CS2)
                price = await client.get_game_prices(steam_appid=271590)
...
class TestSearchGame:
    ...
    async def test_search_game_success(self):
        ...
        # Mock _get_json which is used by search_game
        mock_search_data = [
            {
                "id": "cyberpunk2077",
        ...
        with patch.object(client, '_get_json', new_callable=AsyncMock) as mock_get_json:
            mock_get_json.return_value = mock_search_data

            # Use paid game title instead of F2P
            result = await client.search_game(title="Cyberpunk")
...
class TestSearchDeals:
    ...
    async def test_search_deals_success(self):
        ...
        with patch.object(client._client, 'get', new_callable=AsyncMock) as mock_get:
            ...
            # Use paid game that actually has deals
            deals = await client.search_deals(title="GTA V", limit=20, min_discount=0)
